...
やり取り回数を減らし、1回の指示で完走しやすくするための補助原則を定義する。

- `RULE-INDEX-IMPL-001` 優先順位は「安全性（破壊的変更確認を含む）」を最上位、その次に「やり取り回数を減らす観点」を適用する。
- `RULE-INDEX-IMPL-002` 実装系意図（追加/修正/バグ修正/リファクタ/機能追加/削除/ビルド/テスト/コミット）が含まれる依頼は、原則として計画提示で停止せず実装まで進める。
