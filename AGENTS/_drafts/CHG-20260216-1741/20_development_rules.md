...
- `RULE-SPEECH-START-001` すべての Codex ターンは、処理開始前に必ず開始時読み上げを行う。入力プロンプトを理解した時点で即座に発話し、内容は「今回の依頼内容の要約（1-3文）」と「これから行う作業の種類（認識、整理、設計、更新 等）」を含める。本読み上げは Output Minimal / Resume Minimal / Optimization の制限を受けない。
- `RULE-SPEECH-END-001` すべての Codex ターンは、処理完了後に必ず終了時読み上げを行う。内容は「実際に行った作業内容の要約」と「その結果として期待される効果（手戻り削減、使用量削減、安定化 等）」を含め、実装を行っていない場合でも省略してはならない。
...
- `RULE-SPEECH-FORMAT-001` 読み上げは自然文で簡潔に話す。箇条書き形式を禁止し、読み上げ用途での JSON 形式を禁止する。
