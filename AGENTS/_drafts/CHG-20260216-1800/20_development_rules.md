...
## Resume Minimal Mode (No Source Change Assumption)
前回終了時からソース変更なし前提での最小再開フローを定義する。

- `RULE-DEV-RESUME-MIN-001` 前回終了時からソースコード変更がなく、要件定義書が最新かつ正である前提で再開する場合、Resume Recognition Mode は最小フローを適用する。入力は要件定義書のみとし、対象ファイル一覧は `RULE-DEV-TARGETFILES-001..004` に従って生成し、出力は「対象ファイル一覧（1行1ファイル、辞書順）」と「要件定義書先頭の要約（1-2行）」の2つに限定し、次アクション提案・追加要件生成・長文説明を行わない。
- `RULE-DEV-RESUME-MIN-002` `RULE-DEV-RESUME-MIN-001` の前提を満たす場合、最小再開モードを本来の Resume Recognition Mode より優先し、他の Resume Recognition Mode 出力は参照しない。
- `RULE-DEV-RESUME-MIN-003` `RULE-DEV-RESUME-MIN-001` 適用時は、許可された2出力以外（追加列挙、派生説明、次アクション提案、要件生成）を禁止する。

## Execution Mode Decision Table
判定の揺れを防ぐため、モード決定を単一表で固定する。

- `RULE-DEV-MODEMAP-001` モード判定順序は `ResumeMinimal` -> `ResumeRecognition` -> `Lightweight` -> `Implement` -> `Plan` の順とし、最初に一致したモードを採用する。
- `RULE-DEV-MODEMAP-002` `ResumeMinimal` の成立条件は `RULE-DEV-RESUME-MIN-001` の前提を満たす場合のみとし、成立時は他モード判定を打ち切る。
- `RULE-DEV-MODEMAP-003` 判定結果はターン先頭で1行記録する（例: `mode=ResumeMinimal`）。同一ターン内でモードを変更してはならない。

## Mandatory Speech Phases
開始時と終了時の読み上げを全モードで必須化する。

- `RULE-SPEECH-START-001` すべての Codex ターンは、処理開始前に必ず開始時読み上げを行う。入力理解直後に1文で発話し、依頼要約と作業種類の2要素を含める。本読み上げは Output Minimal / Resume Minimal / Optimization の制限を受けない。
- `RULE-SPEECH-END-001` すべての Codex ターンは、処理完了後に必ず終了時読み上げを行う。1文で発話し、実施内容要約と期待効果の2要素を含める。実装を行っていない場合でも省略してはならない。
- `RULE-SPEECH-PRIORITY-001` 開始時読み上げと終了時読み上げは、Resume Recognition Mode、Resume Minimal Mode、Output 制限、最適化ルール、`Mandatory Development Loop` Step 8 の発話用JSON運用より常に優先される。削除・省略・統合を禁止する。
- `RULE-SPEECH-FORMAT-001` 読み上げは自然文の短文（1-2文）で行う。箇条書き形式を禁止し、読み上げ用途での JSON 形式を禁止する。

## Cost Control Metrics
使用量増加と手戻りの増大を防ぐため、低コスト運用の計測基準を固定する。

- `RULE-DEV-COST-001` 計測指標は `turn_output_lines`（1ターン出力量）、`event_write_count`（発話JSON書込回数）、`same_req_rework_count`（同一 `req_id` の再実装回数）、`nonfunctional_test_runs`（非機能変更でのテスト実行回数）とする。
- `RULE-DEV-COST-002` 目標値は直近運用ベースライン比で `turn_output_lines -40%` `event_write_count -30%` `nonfunctional_test_runs -70%` を既定とし、維持不能な場合は理由を記録する。
- `RULE-DEV-COST-003` ルール更新で上記指標を10%以上悪化させる変更は、ユーザーの明示許可がない限り適用しない。
- `RULE-DEV-COST-004` 計測は各ターンの `after_work` 時点で実施し、同一ターン内の再計測を禁止する。
- `RULE-DEV-COST-005` 計測値は `target_project_root/result.json` の `cost_metrics` に追記し、最低項目として `mode` `turn_output_lines` `event_write_count` `nonfunctional_test_runs` を含める。
- `RULE-DEV-COST-006` ベースラインは直近20ターン移動平均で算出し、更新タイミングを毎ターン終了時に固定する。
...
## Game 2D Minimal Rules（Low-cost）
同人2Dゲーム開発で事故が多い論点を、通常開発コストをほぼ増やさずに規約化する。

- `RULE-GAME2D-ACT-001` ゲーム規約の有効条件は手動フラグ方式とする。プロジェクトルートに `project_kind.txt` が存在し、その内容が `game_2d` の場合のみ本節を適用し、それ以外は適用しない。ディレクトリ走査や雰囲気推測などの自動推測による適用を禁止する。
- `RULE-GAME2D-ACT-002` 有効化判定手順は `project_kind.txt` の1行目を前後空白除去後に `game_2d` と完全一致比較する1ステップのみとし、部分一致や別名許容を禁止する。
- `RULE-GAME2D-ACT-003` `project_kind.txt` が存在しない場合は本節の全ルールを無効とし、推定補完を行わない。
- `RULE-GAME2D-INP-001` ゲーム入力（キーボード/コントローラ等）の受付は、単一の入口関数または単一モジュールに集約する。目的は将来の入力ログ、リプレイ、疑似入力テストへの対応であり、実装を今すぐ必須化しないが、新規実装と改修はこの構造に従う。
- `RULE-GAME2D-INP-002` 入力状態は「フレーム番号または tick」「入力種別」「押下/離し」「キー名またはボタン名」のテキスト表現へ落とせる設計を維持する。目的はリプレイと検証の再現性確保であり、実装は必須化しない。
- `RULE-GAME2D-TIME-001` 可変デルタタイムを使う場合、デルタタイムに上限を必ず設ける（例: `max_dt = 1/30` 秒）。上限超過分は clamp して切り詰め、フレーム落ち時の物理/移動暴走を防ぐ。
- `RULE-GAME2D-TIME-002` 固定タイムステップを採用する場合は、`tick_rate`（例: `60Hz`）を定数定義し、更新処理を tick 単位で進める。本ルールは方針宣言とし、採用/不採用の選択は要件定義書に明記する。
- `RULE-GAME2D-ASSET-001` 画像/音声などのバイナリ自体は対象ソース抽出から除外したまま維持し、代わりに使用アセットを列挙するテキスト宣言ファイルを必須とする。ファイル形式は `assets_manifest.json`（推奨）または `assets_manifest.toml` とし、最小項目は `asset_id` `file_path`（バイナリ相対パス）`usage_tag`（任意）とする。
- `RULE-GAME2D-ASSET-002` 対象ファイル抽出ルールとの統合として、`assets_manifest.json` と `assets_manifest.toml` を include に明示的に含める。バイナリ除外方針は維持し、再開時には仕様アセット情報（manifest）のみを必ず列挙対象に含める。
...
追加ルール:
- `RULE-DEV-LITE-001` 軽微修正例外を定義する。変更内容が軽微（文言修正、コメント追加、フォーマット変更）の場合は、同一要件定義書内へ修正理由を1行で記録し、要件定義書全体の更新を不要とする。軽微修正例外を適用する場合は `AGENTS Change Record` に `req_id` と `brief reason` を必ず記録し、実装コードの機能変更を含めてはならない。
...
## Test Execution Tiers (Waste Guard)
不要テストを抑えつつ品質を維持するため、テスト実行範囲を変更種別で固定する。

- `RULE-DEV-TESTTIER-001` Tier 0（文言修正、コメント追加、フォーマット変更のみ）では、テスト・ビルド・起動検証を実行しない。`N/A` 理由を記録して完了としてよい。
- `RULE-DEV-TESTTIER-002` Tier 1（軽微変更かつ機能変更なし）では、影響する `req_id` に対応する最小の `test_id` のみ実行する。非関連テストの横断実行を禁止する。
- `RULE-DEV-TESTTIER-003` Tier 2（機能変更または挙動変更）では、`public` と `replay` と必要時のGUIスモーク確認を実行する。
- `RULE-DEV-TESTTIER-004` `req_id` に対応しないテスト実行を禁止し、例外はユーザーの明示要求がある場合のみ許可する。
- `RULE-DEV-TESTTIER-005` Tier判定はチェックリスト方式とし、`文言のみ` `コメントのみ` `整形のみ` の全てが真なら Tier 0、`機能変更なし` かつ `設定/配線変更あり` なら Tier 1、それ以外は Tier 2 とする。
- `RULE-DEV-TESTTIER-006` Tier判定結果は `test_tier` と `tier_reason` を1行で記録し、未記録のままテスト実行へ進んではならない。
...
- `RULE-DEV-EVT-007` 副作用がないターン（相談、方針確認、評価、ルール文面確認のみ）では、通常版 `event.json` の毎回出力を要求しない。必要時は `mini_event` 1回でよい。
- `RULE-DEV-EVT-008` イベント出力判定は `side_effect=true/false` の2値で決定する。`side_effect=true` は通常版、`side_effect=false` は `mini_event` 1回を既定とする。
- `RULE-DEV-EVT-009` `side_effect` 判定はターン開始時に1回だけ確定し、途中変更を禁止する。
...
- `RULE-DEV-GATE-001` 実装開始前に `変更範囲` `非範囲` `完了条件` の3点を1回で確定し、確定後は当該サイクル内で凍結する。
- `RULE-DEV-GATE-002` `TBD` が挙動に影響する場合は実装を開始せず、要件確認ターンへ戻す。
- `RULE-DEV-GATE-003` ゲート記録は1行テンプレート `GATE: scope=<...> | non_goals=<...> | done=<...>` を必須とする。
- `RULE-DEV-GATE-004` 上記テンプレートの3項目のいずれかが空の場合は、実装・テストに進んではならない。
...
- ゲートテンプレート（`RULE-DEV-GATE-003`）をこの時点で必ず1行記録する。
...
### 2. Test Design First
- テスト設計と実行対象の決定は `RULE-DEV-TESTTIER-001..004` を優先する（Tier 0 は `N/A` 記録で省略可）。
...
- 検証実行範囲は `RULE-DEV-TESTTIER-001..004` に従い、合否を記録する。
...
- 作業開始前（`phase="before_work"`）と作業完了の最後（`phase="after_work"`）に、「現在の読み上げ対象JSON」へ出力する:
  ...
  - この要件は `RULE-DEV-EVT-001` に該当するターンで必須適用する。
  - 通常の会話応答のみのターンは `RULE-DEV-EVT-002..007` を優先し、通常版の毎回出力を要求しない。
...
Change Record の肥大化を防ぎ、可読性を維持する。

- `RULE-DEV-CHGREC-001` 次回更新以降、`AGENTS/20_development_rules.md` の `AGENTS Change Record` には直近3件のみを保持する。
- `RULE-DEV-CHGREC-002` 次回更新以降は、4件目以降の履歴を `AGENTS/_snapshots/CHANGELOG.md` へ移送して保持する。移行完了まで既存履歴の暫定共存を許可する。
- `RULE-DEV-CHGREC-003` 新規追加レコードには必ず「1行要約」の `notes` を記載する。
...
- `change_id`: `CHG-20260216-1717`
- `target_files`: `AGENTS/20_development_rules.md`
- `rules_added_or_updated`: `RULE-DEV-LITE-001`, `RULE-DEV-RESUME-MIN-001`, `RULE-DEV-RESUME-MIN-002`
- `validation_result`: `PASS`（参照先存在・見出し存在・ルールID重複なし・優先順位矛盾なし）
- `rollback_point`: `C:\Users\gonec\codex_workspace\AGENTS\_snapshots\CHG-20260216-1717\20_development_rules.md`
- `notes`: `軽微修正例外例 & 最小再開モードを規約化`
- `change_id`: `CHG-20260216-1724`
- `target_files`: `AGENTS/20_development_rules.md`
- `rules_added_or_updated`: `RULE-SPEECH-START-001`, `RULE-SPEECH-END-001`, `RULE-SPEECH-PRIORITY-001`, `RULE-SPEECH-FORMAT-001`
- `validation_result`: `PASS`（参照先存在・見出し存在・ルールID重複なし・優先順位矛盾なし）
- `rollback_point`: `C:\Users\gonec\codex_workspace\AGENTS\_snapshots\CHG-20260216-1724\20_development_rules.md`
- `notes`: `開始終了読み上げを必須フェーズとして昇格`
- `change_id`: `CHG-20260216-1741`
- `target_files`: `AGENTS/20_development_rules.md`
- `rules_added_or_updated`: `RULE-GAME2D-ACT-001`, `RULE-GAME2D-INP-001`, `RULE-GAME2D-INP-002`, `RULE-GAME2D-TIME-001`, `RULE-GAME2D-TIME-002`, `RULE-GAME2D-ASSET-001`, `RULE-GAME2D-ASSET-002`
- `validation_result`: `PASS`（参照先存在・見出し存在・ルールID重複なし・優先順位矛盾なし）
- `rollback_point`: `C:\Users\gonec\codex_workspace\AGENTS\_snapshots\CHG-20260216-1741\20_development_rules.md`
- `notes`: `同人2Dゲーム向け低コスト規約（入力窓口/時間管理/アセット宣言）追加。自動推測はせず手動フラグで適用`
- `change_id`: `CHG-20260216-1750`
- `target_files`: `AGENTS/20_development_rules.md`, `AGENTS/30_output_rules.md`
- `rules_added_or_updated`: `RULE-DEV-RESUME-MIN-003`, `RULE-SPEECH-START-001`, `RULE-SPEECH-END-001`, `RULE-SPEECH-FORMAT-001`, `RULE-DEV-COST-001..003`, `RULE-DEV-TESTTIER-001..004`, `RULE-DEV-EVT-007`, `RULE-DEV-GATE-001..002`, `RULE-OUT-COST-001..003`
- `validation_result`: `PASS`（参照先存在・見出し存在・ルールID重複なし・優先順位矛盾なし）
- `rollback_point`: `C:\Users\gonec\codex_workspace\AGENTS\_snapshots\CHG-20260216-1750\`
- `notes`: `使用量増加・手戻り・不要テストを抑える運用へ再構成（発話短文化、最小再開固定、テスト階層化、イベント出力最適化）`
- `change_id`: `CHG-20260216-1800`
- `target_files`: `AGENTS/20_development_rules.md`, `AGENTS/30_output_rules.md`
- `rules_added_or_updated`: `RULE-DEV-MODEMAP-001..003`, `RULE-DEV-COST-004..006`, `RULE-GAME2D-ACT-002..003`, `RULE-DEV-TESTTIER-005..006`, `RULE-DEV-EVT-008..009`, `RULE-DEV-GATE-003..004`, `RULE-DEV-CHGREC-001..003`, `RULE-OUT-COST-004..005`
- `validation_result`: `PASS`（参照先存在・見出し存在・ルールID重複なし・優先順位矛盾なし）
- `rollback_point`: `C:\Users\gonec\codex_workspace\AGENTS\_snapshots\CHG-20260216-1800\`
- `notes`: `発話必須は維持しつつ、モード判定表・測定手順・Tier判定・二値イベント・ゲート定型・出力定型を追加`
