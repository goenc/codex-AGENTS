...
- `RULE-DEV-CONFIG-008` Step 4 では設定形式の静的検査を必須実行し、最低限 `assets/config/*.txt` の存在、ソース/ビルドスクリプト内の `config/*.txt` 参照、`assets/config/*.default.json` の欠落を機械検出する。
- `RULE-DEV-CONFIG-009` `RULE-DEV-CONFIG-008` で違反が出た場合は実装完了判定を不可とし、修正完了まで `User Manual Gate` へ進まない。
