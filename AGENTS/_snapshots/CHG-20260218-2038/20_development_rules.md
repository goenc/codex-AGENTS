...
- `RULE-DEV-TARGETFILES-002` include の既定は、ソース `*.rs`、ドキュメント `*.md` `*.txt`、設定/データ `*.json` `*.yaml` `*.yml` `*.toml` `*.ini`、スクリプト `*.ps1` `*.bat`、フォント `*.ttf` `*.otf` とする。
  - Rust必須: `Cargo.toml` `Cargo.lock`
  - 典型: `.cargo/config.toml`（存在する場合）
  - テスト: `tests/` 配下、または `src/` 内の `mod tests` を含む `*.rs`
  - exclude の既定は、フォルダ `.git/` `target/` `node_modules/` `dist/` `build/` `logs/` `runs/` `tmp/`、ファイル `*.log` `*.tmp` `*.ttc`、バイナリ全般（`*.png` `*.jpg` `*.jpeg` `*.webp` `*.gif` `*.bmp` `*.ico` `*.mp3` `*.wav` `*.mp4` `*.mov` `*.avi` `*.zip` `*.7z` `*.rar` `*.exe` `*.dll` `*.pdb` `*.pdf` など）とする。秘密情報 `.env` `*.key` `*.pem` `*token*` `*secret*` `*credential*` は常に除外する。
...
- `RULE-DEV-LOOP-007` `OK` 判定後、次サイクルの要求を `要件定義_プロジェクト名.md` の「要件定義」へ追記または更新する。
- `RULE-DEV-LOOP-008` 次サイクル要求が未確定の場合は「未確定」と明示してから Step 8 に進む。

### 8. Record & Handoff
- `RULE-DEV-RESULT-001` `target_project_root/result.json` には `result_mode` を必須出力し、値は `minimal` または `full` とする。
- `RULE-DEV-RESULT-002` 既定は `minimal` とし、`minimal` では `result_mode` `os` `rustc_version` `cargo_version` `timestamp_utc` のみを必須とする。
- `RULE-DEV-RESULT-003` `full` は、ユーザーが明示要求した場合、リリース引き渡し時、または失敗再現性確保が必要な場合に適用する。
- `RULE-DEV-RESULT-004` `full` では `RULE-DEV-RESULT-002` の項目に加えて `cpu_arch` `target_triple` `locale` `timezone` `dependency_set`（`Cargo.lock` がある場合はその存在記録を含む）を必須とする。
